...
/**
 * Short proof points pulled out of each description. Full key paths rather than
 * leaf names, so the type checker sees only real combinations: building them as
 * `${key}.features.${leaf}` would also admit pairings that do not exist.
 */
const features = {
  motorcycle: [
 ...
 *
 * This is the one dark section on the page. Everything around it is cream, so
 * the tonal flip gives the page a spine and lets the photography carry weight
 * it cannot carry on a light background.
 */
export async function Experiences() {
  ...
  return (
    <section className="relative overflow-hidden bg-brand-950 py-24 text-cream-100 sm:py-32">
        ...
        <div data-anim="up">
          <div className="grid gap-8 lg:grid-cols-12 lg:gap-16">
            ...
            <div className="lg:col-span-5 lg:pt-20">
              <p className="text-[15px] leading-[1.85] text-pretty text-cream-100/60 sm:text-[16px]">
                {t("body")}
              </p>
            </div>
          </div>
              ...
              >
                ...
                >
                  <Link
                    ...
                    tabIndex={-1}
                    aria-hidden
                    className="group relative block overflow-hidden rounded-[28px] ring-1 ring-cream-100/15"
                  >
                    ...
                    <span className="font-display absolute top-5 left-6 text-[64px] leading-none font-extrabold text-cream-100/85 tabular-nums sm:text-[84px]">
                ...
                >
                  ...
                  <p className="mt-4 text-[14.5px] leading-[1.8] text-pretty text-cream-100/55">
                  ...
                  <ul className="mt-7 flex flex-wrap gap-2">
                    {features[experience.key].map((feature) => (
                      <li
                        key={feature}
                        className="rounded-full border border-cream-100/18 px-3.5 py-1.5 text-[10.5px] font-semibold tracking-[0.13em] text-cream-100/75 uppercase"
                      >
                        {tw(feature)}
                      </li>
                    ))}
                  ...
                  <Link
                    href={experience.href}
                    className="group mt-8 inline-flex items-center gap-3 text-[11px] font-bold tracking-[0.16em] text-cream-100 uppercase"
                  >
                    <span
                      aria-hidden
                      className="h-px w-8 bg-ember-500 transition-all duration-500 ease-out-expo group-hover:w-14"
                    />
                    {tw("cta")}
