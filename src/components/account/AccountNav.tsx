...
/**
 * The section nav, marking the deepest match rather than every prefix, so
 * "My profile" does not stay lit while you are on Bookings.
 */
export function AccountNav() {
  ...
  return (
    <nav
      aria-label="Account"
      className="-mb-px flex gap-1 overflow-x-auto border-b border-brand-900/12 pt-10"
    >
      {TABS.map((tab) => {
          ...
          <Link
            ...
            className={`border-b-2 px-4 py-3 text-[12px] font-bold tracking-[0.1em] whitespace-nowrap uppercase transition-colors ${
              current
                ...
                : "border-transparent text-brand-800/45 hover:text-brand-900"
            }`}
          >
            {tab.label}
          </Link>
